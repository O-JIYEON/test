...
### 2.3.5 components/common 기능별 폴더 규칙

`components/common` 내부는 **기능 단위 폴더 구조**를 사용한다.

- 하나의 UI 컴포넌트 = 하나의 폴더
- 컴포넌트, 스타일, 내부 구성 요소를 같은 폴더에서 관리
- 파일명은 컴포넌트명과 일치하도록 유지
- 매우 단순하고 확장 가능성이 낮은 UI의 경우,
  프로젝트 기존 컨벤션을 우선하여 단일 파일로 유지할 수 있다.
- 사용자 상태(로딩, 성공, 실패, 알림)를 표현하는 UI는 `feedback/`에 둔다.
